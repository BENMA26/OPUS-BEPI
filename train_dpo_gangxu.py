"""
DPO fine-tuning of GraphBepi with ESM GangXu + FoldSeek tokens and spatial-coherence preference.
...
python train_dpo_gangxu.py \
    --ref_ckpt ./model/BCE_633_GraphBepi_gangxu/model_-1.ckpt \
    --dataset  BCE_633 --tag GraphBepi_dpo_gangxu \
...
from dataset import PDB_DPO_gangxu, collate_fn_dpo
...
if __name__ == '__main__':
    ...
    parser.add_argument('--feat_dim',     type=int, default=2581,
                        help='input feature dim (2560 ESM + 21 FoldSeek tokens).')
    ...
    # ── Datasets ──────────────────────────────────────────────────────────────
    trainset = PDB_DPO_gangxu(mode='train', fold=args.fold, root=root)
    valset   = PDB_DPO_gangxu(mode='val',   fold=args.fold, root=root)
    testset  = PDB_DPO_gangxu(mode='test',  fold=args.fold, root=root)
