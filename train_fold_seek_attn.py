...
from dataset import PDB,PDB_foldseek,PDB_foldseek_tokens,PDB_foldseek_attn,collate_fn,collate_fn_fold_tokens,chain
...
trainset=PDB_foldseek_attn(mode='train',fold=args.fold,root=root)
valset=PDB_foldseek_attn(mode='val',fold=args.fold,root=root)
testset=PDB_foldseek_attn(mode='test',fold=args.fold,root=root)

train_loader=DataLoader(trainset,batch_size=args.batch,shuffle=True,collate_fn=collate_fn_fold_tokens,drop_last=True)
val_loader=DataLoader(valset,batch_size=args.batch,shuffle=False,collate_fn=collate_fn_fold_tokens)
test_loader=DataLoader(testset,batch_size=args.batch,shuffle=False,collate_fn=collate_fn_fold_tokens)
if args.fold==-1:
...
model=GraphBepi_att(
    feat_dim=2581,                     # esm2 representation dim
