...
class chain:
    ...
    def extract(self,model,device,path):
        if len(self)>1024 or model is None:
            return
        f=lambda x:model(x.to(device).unsqueeze(0),[33])['representations'][33].squeeze(0).cpu()
        with torch.no_grad():
            feat=f(self.amino)
        torch.save(feat,f'{path}/feat/{self.name}_esm2.ts')
...
def collate_fn_fold_tokens(batch):
    edges = [item['edge'] for item in batch]
    fold_token = [item['fold_token'] for item in batch]
    feats = [item['feat'] for item in batch]
    labels = torch.cat([item['label'] for item in batch],0)
    return fold_token,feats,edges,labels
...
def initial(file, root, model=None, device='cpu', from_native_pdb=True):
    ...
    # ✅ 新增：准备保存 pid 的文件
    pid_txt_path = os.path.join(root, "pid_list.txt")
    with open(pid_txt_path, 'w') as pid_file:
        with tqdm(prefix) as tbar:
            for i in tbar:
                tbar.set_postfix(protein=i)
                pid = i[:4]       # 取 PDB ID
                ...
                if from_native_pdb:
                    ...
                    state = extract_chain(root, pid, chain_id)
                    if not state:
                        continue
    ...
    print(f"\n✅ 提取完成，共写入 {len(samples)} 个样本。")
    print(f"✅ 已保存 PDB ID 列表到：{pid_txt_path}")
'''
